...
if(sitename!="") 
	#site_files.each do |site_filename|
	  ...
	  site = Psych.load_file("sites/#{sitename}.yaml")
	  ...
	  puts "\t#{sitename.upcase} \t :: Generating feature files, scenarios and step definition code"
	  ...
	  template_files.each do |template_filename|
		if File.file?(template_filename)
			...
			if (output_content =~ /\A#ignore/) then
				...
				puts "\tCreated file \t  : #{output_filename}"
			end
		end    
	  end
